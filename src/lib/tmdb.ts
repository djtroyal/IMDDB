...
async function getPersonDetails(personId: number): Promise<{
  ...
  if (!res.ok) return { birthday: null, deathday: null, biography: null };
  const data = await res.json();
  return {
...
export async function getCastWithDetails(movieId: number): Promise<CastMember[]> {
  ...
  // Batch person details in groups of 10
  const BATCH = 10;
  const enriched: CastMember[] = [];

  for (let i = 0; i < rawCast.length; i += BATCH) {
    const batch = rawCast.slice(i, i + BATCH);
    const details = await Promise.all(batch.map((m) => getPersonDetails(m.id)));
